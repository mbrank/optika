...
//takes material struct and color
{
  ...
  case 4: {
	// emitter
	// emitter material does not reflect anything, it just
	// tells the ray what color it is and performs no
	// reflection
	mat->scattered.origin = rec->p;  //ray(rec.p, scatter_direction);
    mat->scattered.direction.x = 0;
    mat->scattered.direction.y = 0;
	mat->scattered.direction.z = 0;
	mat->attenuation.x = mat->emitter.x;
	mat->attenuation.y = mat->emitter.y;
	mat->attenuation.z = mat->emitter.z;
	mat->albedo.x = mat->emitter.x;
	mat->albedo.y = mat->emitter.y;
	mat->albedo.z = mat->emitter.z;
